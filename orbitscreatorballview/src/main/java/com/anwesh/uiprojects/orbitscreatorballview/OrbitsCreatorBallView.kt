...
fun Canvas.drawOrbitCreatorBall(scale : Float, w : Float, paint : Paint) {
    ...
    val gap : Float = (end - start) / circles
    var si1 : Float = 0f
    var si2 : Float = 0f
    val ballR : Float = w * ballRFactor
    for (j in 0..(circles - 1)) {
        val r  : Float = start + j * gap
        val sfi : Float = sf.divideScale(1, 2).divideScale(j, circles)
        if (sfi <= 0f) {
            break
        }
        ...
        drawArc(RectF(-r, -r, r, r), -90f, 360f * sfi1, false, paint)
    }
    paint.style = Paint.Style.FILL
    save()
    rotate(360f * si1)
    drawCircle(0f, -(start + gap * si2), ballR * sf.divideScale(0, 2), paint)
    restore()
}
